...
from . import make_mention, LOGS, ultroid_cmd, ultroid_bot
...
async def watch_edits(chat, msg_id, timeout=16):
    async with ultroid_bot.conversation(chat, timeout=timeout) as conv:
        func = lambda e: e.id == msg_id and search(
            rf"(?i)Current turn: (.+){ultroid_bot.uid}", e.message.text
        )
        response = conv.wait_event(
            MessageEdited(
                incoming=True,
...
async def autohexa(e):
    ...
    for _ in range(1, count):
        try:
            ...
            LOGS.exception(exc)
            await domt.reply(f"Got {exc.class} in run #{_} \n\n`{exc}`")
            continue
        else:
            success += 1
            await domt.edit(f"Finished run #{_}")
        finally:
            # sleep between runs
